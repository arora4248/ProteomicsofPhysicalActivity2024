...
# j is avector for number of PA traits
j=10

## Out is a mterix for results output
...
for (i in 1:length(protein_files)){
  ...
  if (nrow(protein>0)){
    ...
    possibleError<-tryCatch(tmp<- ld_clump(
      ...
      bfile = "filepath_local_Rpackage/TwoSampleMR_local_ld/EUR"
    ), error=function(e) e)
    ...
    if(nrow(exp2>0)) {
      ...
      if (nrow(dat)>0) {
        ...
        mr_results5 <- mr(dat, method_list = c("mr_simple_median", "mr_wald_ratio"))
        mr_results5
...
write.csv(Outord,paste0(file="filepath_to_output_directory/MR_",gwas_files[j],"_on_2940proteins.csv"),row.names=F,quote=F)
